 ...
 */
cfcli.findRecords = function (name, value, result) {
  return _.filter(
    result,
    function (row) {
      var match = row.name == name;
      if (match && value) {
        match = row.content == value
      }
...
cfcli.addrecord = function (options) {
  if (options._.length < 3) {
    ...
    if (options.type === 'SRV') {
      opts = _.merge(opts, processSrvOptions(options));
    }
...
cfcli.editrecord = function (options) {
  var editRecord = function (err, result) {
    ...
    var matches = cfcli.findRecords(name, false, result);
    if (matches.length === 1) {
      var match = matches[0];
      ...
      var opts = {
        ...
        prio: options.priority || match.prio || 0
      };
      if (options.type === 'SRV') {
        opts = _.merge(opts, processSrvOptions(options));
      }
      ...
      console.log(util.format('Found %d records that matched %s, cancelling edit', matches.length, name));
    } else {
      console.log(util.format('No records found that match %s', name));
    }
...
cfcli.removerecord = function (options) {
  var removeRecord = function (err, result) {
    var name = cfcli.prepareName(options._[1], options.domain);
    var value = options._[2] || false;
    var matches = cfcli.findRecords(name, value, result);
    if (matches.length > 0) {
    ...
    if (err) {
      ...
      result.forEach(function (val) {
        data.push([
          val.type,
          val.display_name,
          val.content, (val.ttl == 1) ? 'Auto' : val.ttl, ((val.service_mode == 1) ? 'Active' : 'Inactive')
        ]);
      });
      var tableConfig = {
  ...
  cfcli.cloudflare.listDomains(function (err, result) {
    if (err) {
      ...
      result.forEach(function (val) {
        data.push([
          val.display_name,
          val.props.plan.replace('_', ' '), ((val.zone_mode == 1) ? 'Active' : 'Inactive')
        ]);
      });
...
function processSrvOptions(options) {
  var srvOptions = {};
  var serverParts = options._[1].split('.');
  srvOptions.service = serverParts[0];
  srvOptions.protocol = serverParts[1];
  srvOptions.srvname = serverParts[2];
  srvOptions.prio = options._[2];
  srvOptions.weight = options._[3];
  srvOptions.port = options._[4];
  srvOptions.target = options._[5];
  srvOptions.content = util.format('%s %s %s %s',
    srvOptions.prio,
    srvOptions.weight,
    srvOptions.port,
    srvOptions.target
