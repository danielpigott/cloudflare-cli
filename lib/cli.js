...
cfcli.editrecord = function(options) {
  var editRecord = function(err, result) {
    ...
    if (matches.length === 1) {
      ...
      console.log(util.format('Found %d records that matched %s, cancelling edit', matches.length, match.name));
    } else {
      console.log(util.format('No records found that match %s', match.name));
    }
