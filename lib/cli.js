...
cfcli.toggledev = function(options) {
    var toggle = (options._[1] === 'on')? true: false;
    if(!options._[1] || !options._[1].match(/on|off/)) {
        console.log("Please use on/off after toggledev command.");
        return;
    }
    cfcli.cloudflare.devmode(options.domain, toggle, function(err, result){
        if(err) {
            ...
            console.log("Toggle Development mode " + options._[1] + " for domain" + options.domain);
        }
    });
}
